...
def main():
	...
	pprint.pprint(graph_bi)
	print(f"start: {pel_start}, end: {pel_end}.")


	def rec_search(head,tail,distance,depth=0):
		available_edges=[edge for edge in graph_bi[head] if edge.pelid_target not in tail]
		if len(available_edges)==0:
			# No more path left.
			if head==pel_end.id:
				# print(("-"*depth)+f"{head},{tail},{distance}")
				return distance
		else:
			dists=[]
			for edge in available_edges:
				tail.append(head)
				# print(("-"*depth)+f"PRE  {head=},{dists=}")
				dists.append(rec_search(edge.pelid_target,tail,distance+edge.distance,depth+1))
				# print(("-"*depth)+f"POST {head=},{dists=}")
				tail.pop(-1)
			# print(("-"*depth)+f"result: {dists}")
			return max(dists)
		return 0
