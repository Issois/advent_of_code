...
def main():
	...
	for idx,row in enumerate(inp):
		...
		for gcnt in np.unique(gcnt_broken):
			...
			for idx_poss_space in idx_posps:
				if idx_poss_space>(spring_count-gcnt):
					break
				not_intersect_func=np.all(springs[idx_poss_space:idx_poss_space+gcnt]<FUNCTIONAL)
				not_touch_brok_start=idx_poss_space==0 or springs[idx_poss_space-1]>BROKEN
				not_touch_brok_end=idx_poss_space==spring_count-gcnt or springs[idx_poss_space+gcnt]>BROKEN
				is_poss_space=not_intersect_func and not_touch_brok_start and not_touch_brok_end
				# print(f"{gcnt=},{idx_poss_space=},{not_intersect_func=},{not_touch_brok_start=},{not_touch_brok_end=}")
				# print(is_poss_space)
				if is_poss_space:
					idx_posps_from_gcnt[gcnt].append(idx_poss_space)
...
def get_possibilities(
	...
	possibilities=0
	for idx_poss_space in idx_posps:
		if posp_idx_lo<=idx_poss_space<=posp_idx_hi-1-gcnt:
			if PRINT:
				print(f"{('-'*depth).ljust(5)}LOOP    {marked_range(springs,idx_poss_space)} {marked_range(gcnt_broken,gcnt_idx_center)}")
			poss_lo=1
			poss_hi=1
